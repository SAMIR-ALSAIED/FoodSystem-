...
<header class="d-flex justify-content-between align-items-center bg-dark p-2">
    <div>
        <h1 class="text-white m-0">شاشة الكاشير</h1>
            ...
            <div class="row g-2" id="products-grid">
                ...
                <div class="col-6 col-md-4">
                    <div class="product-card position-relative p-2 border rounded"
                        ...
                        <!-- عدد المنتج -->
                        <span class="product-qty badge bg-danger position-absolute top-0 start-0 m-1 d-none">0</span>

                        <h6 class="text-center">{{ $product->name }}</h6>
                        <p class="text-center mb-0">{{ $product->price }} ج</p>
                    </div>
                </div>
                @endforeach
            </div>
                        ...
                        <div class="col-6">
                            <div class="card shadow-sm border-0 bg-success bg-gradient text-white">
                                <div class="card-body d-flex align-items-center gap-2 p-2">
                                    <span class="fs-5"><i class="bi bi-wallet2"></i></span>
                                    <div class="flex-fill">
...
// تحديث عداد المنتج على الكارت
function updateProductBadge(card, qty){
    const badge = card.querySelector('.product-qty');
    if(qty > 0){
        badge.innerText = qty;
...
function renderOrder(){
    ...
    document.querySelectorAll('.qty-input').forEach(input=>{
        input.addEventListener('input', ()=>{
            ...
            const item = orderItems[input.dataset.index];
            item.quantity = value;

            const card = document.querySelector(`.product-card[data-id="${item.id}"]`);
            updateProductBadge(card, value);
...
document.querySelectorAll('.product-card').forEach(card=>{
    card.addEventListener('click', ()=>{
        ...
        let exist = orderItems.find(i => i.id === id);
        if(exist){
            exist.quantity++;
            updateProductBadge(card, exist.quantity);
        } else {
            orderItems.push({id,name,price,quantity:1});
            updateProductBadge(card, 1);
        }
...
<head>
    ...
    <style>
        ...
        tfoot td { font-weight:bold; font-size:16px; border-top:2px solid #000; }

        .totals { width:100%; margin-top:10px;  }
        .totals td { font-weight:bold; padding:6px 10px; font-size:15px; }
...
<body>
    <div class="header">
        ...
        <p>فاتورة رقم </p>
        <p>${invoiceDate} </p>
        <p><strong> اسم الكاشير:</strong> {{ auth()->user()->name }}</p>
